 ...
 */
static void pump_ex(int input, int output, int close_output, int log_fd) {
    char buf[4096];
    int len;
	int written;
	memset(buf, 0, 4096);
    while ((len = read(input, buf, 4096)) > 0) {
		...
		LOGD("written: %d %s", len, buf);
		//if (log_fd == -1) {
		   //write(output, buf, len);
		//}
		//LOGD("ptmx output: %s", buf);
		if (log_fd != -1) {
			//LOGD("ptmx output: %s", buf);
		    written = write(log_fd, buf, len);
	    }
		memset(buf, 0, 4096);
			/*if () {
				
			}*/
			//if (write_blocking(log_fd, buf, len) == -1) {  
	...
	// WK, line disabled on 25/02/2023: never close STDIN_FINO: this will break continue reading next time we reach the loop again
    // close(input);
 ...
 * output FD when done.
 */
static void pump(int input, int output, int log_fd) {
    pump_ex(input, output, 1, /*-1,*/ log_fd);
}
...
static void* pump_thread(void* data) {
    ...
    int output = files[1];
	int log_fd = files[2];
    pump(input, output, log_fd);
    free(data);
    return NULL;
}

static void pump_async(int input, int output, int log_fd) {
    pthread_t writer;
 ...
 */
void pump_stdin_async(int outfd, int log_fd) {
    // WK, on 13/03/2023: enabled again: put the stdout into raw mode so that each character is wrriten independently of pressing enter "\n" on the Terminal Emulator app.
	/* WK: disabled on 13/03/2023: this is being called on the wrong side of the wire:
    * The Terminal stdin's attributes is already set, but we try to set it again, leaving it into an unexpected state after using pipe commands like 
	* su -c ps | grep su.
   ...
   // Put stdin into raw mode
    ...
    pump_async(STDIN_FILENO, outfd, log_fd);
}
...
void pump_stdout_blocking(int infd, int log_fd) {
    ...
    pump_ex(infd, STDOUT_FILENO, 0 /* Don't close output when done */, log_fd);
